const Sequelize = require('sequelize'); // Sequlize 모듈

// Comment 클래스를 Sequelize 모델을 상속 받아 정의
class Comment extends Sequelize.Model {
    // initate 메서드 정의 -> 데이터베이스 연결 초기화
    static initiate(sequelize) {
        ...
        Comment.init({
            ...
            sequelize, // Sequlize 인스턴스 지정
            timestamps: false, // created At 및 updatedAt 필드를 자동으로 생성하지 않음.
            modelName: 'Comment', // 모델 이름 지정
            tableName: 'comments', // 테이블 이름 지정
            paranoid: false, // 소프트 삭제를 사용하지 않음.
                                // ture로 설정 -> 로우를 삭제할 때 완전히 지워지지 않고 deletedAt에 지운 시각이 기록됨(로우 복원 가능)
            charser: 'utf8mb4', // 문자셋을 UTF-8MB4(4바이트 유니코드 / 기본문자 + 특수기호 + 이모티콘으로 구성)로 지정
