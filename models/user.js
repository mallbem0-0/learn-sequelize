...
class User extends Sequelize.Model {
    ...
    static initiate(sequelize) {
        ...
        User.init({
            ...
            married: { // 결혼 여부 속성 정의
                type: Sequelize.BOOLEAN, // 부울 타입(treu/false)
                allowNull: false, //null 허용하지 않음.
            },
            comment: { // 코멘트 속성 정의
            ...
            underscored: false, // 카멜 케이스(예시: created_at) 컬럼.테이블명 사용
                                // true로 설정 -> 스네이크 케이스(예시: created_at) 컬럼.테이블명 사용
            modelName: 'User', // 모델 이름
            tableName: 'users', // 실제 데이터베이스 테이블 이블-> 모델이름이 User인 경우 테이블 이름은 users
            paranoid: false, // 소프트 삭제 비활성화 
                                // true로 설정 -> 로우를 삭제할 때 완전히 지워지지 않고 deletedAt에 지운 시각이 기록됨(루우 복원 가능)
            charset: 'utf8', // 문자 인코딩
            collate: 'utf8_general_ci', // 문자 정렬
                                        // 한글 입력 -> charser: 'utf8', collate: 'utf8_general_ci'로 설정
                                        // 한글과 이모티콘 입력 -> charser: 'utf8mb4', collate: 'utf8mb4_general_ci'로 설정
        });
    }
    // 다른 모델과의 관계 정의
    static associate(db) {
